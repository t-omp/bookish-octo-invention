...
# ───────────────────────────────────────────────────────────────────────────────
# Reliable app list (Waydroid bug workaround)
# ───────────────────────────────────────────────────────────────────────────────
get_app_list() {
    local tries=0
    while (( tries < 10 )); do
...
install_fdroid() {
    ...
    local version_code
    version_code=$(curl -fsSL "${FDROID_METADATA}/${pkg}" | jq -r '.suggestedVersionCode // .packages[].versionCode' | head -n1)
...
for entry in "${APKS[@]}"; do
    ...
    case "$source_type" in
        fdroid)  install_fdroid "$pkg" || failed+=("$pkg") ;;
        url)     install_url "$pkg" "${source#url|}" || failed+=("$pkg") ;;
        bundled) install_bundled "$pkg" || failed+=("$pkg") ;;
        *)       log "Unknown source '$source_type' for $pkg"; failed+=("$pkg") ;;
    esac
done
